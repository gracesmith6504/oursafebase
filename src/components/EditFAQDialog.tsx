...
interface EditFAQDialogProps {
  ...
  faq: FAQ | null;
  onSuccess: (id: string, question: string, answer: string) => void;
}
...
export function EditFAQDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      onSuccess(faq.id, question.trim(), answer.trim());
      onOpenChange(false);
    } finally {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[525px]">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <DialogFooter>
            <Button type="submit" disabled={isSubmitting || !question.trim() || !answer.trim()} className="w-full">
