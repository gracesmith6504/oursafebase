...
export const EditCoCDialog = ({ open, onOpenChange, coc, onSuccess }: EditCoCDialogProps) => {
  ...
  const clearFile = () => {
    setSelectedFile(null);
    const fileInput = document.getElementById("edit-coc-file-upload") as HTMLInputElement;
  ...
  const handleUpdate = async () => {
    ...
    if (uploadMode === "file" && selectedFile) {
      if (currentFileUrl && coc.society_id) {
        ...
        if (oldFilePath) {
          await supabase.storage.from("code-of-conduct-files").remove([oldFilePath]);
        }
      }
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from("code-of-conduct-files").getPublicUrl(filePath);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>Update the content of this template.</DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <Tabs value={uploadMode} onValueChange={(v) => setUploadMode(v as "text" | "file")}>
              ...
              <TabsContent value="text" className="mt-4">
                ...
                />
                <p className="text-xs text-muted-foreground mt-2">Characters: {content.length} / 50,000</p>
              </TabsContent>
              ...
              <TabsContent value="file" className="mt-4">
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-2 p-3 bg-accent rounded-lg">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium">
                          Current file: {getFileExtension(currentFileUrl).toUpperCase()}
                        </p>
                  ...
                  <label className="file-upload-area">
                    ...
                    <div className="text">
                      <span>
                        {currentFileUrl ? "Click to upload new file" : "Click to upload file"} (PDF, DOC, DOCX, TXT,
                        Images)
                      </span>
                    </div>
                    <input
                      id="edit-coc-file-upload"
                      type="file"
                  ...
                  <p className="text-xs text-muted-foreground">
                    The file must be less than 10MB and in PDF, DOC, DOCX, TXT, JPG, PNG, or GIF format.
                  </p>
                </div>
              </TabsContent>
            </Tabs>
