...
-- ============================================
-- PART 1: SOCIETIES TABLE - Combine 3 UPDATE policies into 1
-- ============================================
...
USING (
  ...
  OR
  -- Committee can update (but not creator_email or is_verified)
  is_committee_member((select auth.uid()), id)
  OR
  -- Creator can update (but not is_verified)
...
WITH CHECK (
  ...
  has_role((select auth.uid()), 'admin'::app_role)
  OR
  -- Committee can update but must preserve creator_email and is_verified
  (
    is_committee_member((select auth.uid()), id)
  ...
  OR
  -- Creator can update but must preserve is_verified
  (
    is_society_creator((select auth.uid()), id)
    AND is_verified = (SELECT s.is_verified FROM societies s WHERE s.id = societies.id)
  )
...
-- ============================================
-- PART 4: USER_ROLES TABLE - Combine 2 SELECT policies into 1
-- ============================================

DROP POLICY IF EXISTS "Only admins can manage roles" ON public.user_roles;
DROP POLICY IF EXISTS "Users can view own roles" ON public.user_roles;

-- Combined optimized SELECT policy
...
-- Recreate admin management policies with optimization
